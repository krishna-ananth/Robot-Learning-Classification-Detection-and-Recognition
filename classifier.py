...
class Classifier:
    ...
    def train(self, svm_kernel, codebook, des_option=constants.ORB_FEAT_OPTION, is_interactive=True):
        ...
        if is_interactive:
            dataOption = input("Enter [1] to calculate VLAD vectors for the training set or [2] to load them.\n")
        else:
            dataOption = constants.GENERATE_OPTION
        if dataOption == constants.GENERATE_OPTION:
            ...
            self.log.train_vlad_time(endTime - startTime)
        else:
            ...
            x = nmpy.matrix(x, dtype=nmpy.float32)
        svm = cv2.SVM()
        ...
        if svm_option == constants.GENERATE_OPTION:
            ...
            endTime = time.time()
            self.log.svm_time(endTime - startTime)
    ...
    def test(self, codebook, svm, des_option = constants.ORB_FEAT_OPTION, is_interactive=True):
        ...
        if is_interactive:
            dataOption = input("Enter [1] to calculate VLAD vectors for the testing set or [2] to load them.\n")
        else:
            dataOption = constants.GENERATE_OPTION
        if dataOption == constants.GENERATE_OPTION:
            ...
            startTime = time.time()
	    #print self.dataset.get_test_set()
            ...
            self.log.test_vlad_time(endTime - startTime)
        else:
        ...
        endTime = time.time()
        self.log.predict_time(endTime - startTime)
        mask = result == y
        correctCalc = nmpy.count_nonzero(mask)
        accuracy = (correctCalc * 100.0 / result.size)
